...
fn jelly_terminal_thread(sender: &Sender<Event>) {
    let mut buffer = String::new();
    loop {
        if let Ok(len) = stdin().read_line(&mut buffer) {
            if len == 0 {
...
fn create_jelly_terminal_thread(sender: Sender<Event>) -> JoinHandle<()> {
    thread::spawn(move || jelly_terminal_thread(&sender))
}

pub fn event_loop_headless_jelly(
    event_channel: &Receiver<Event>,
    event_sender: Sender<Event>,
    hardware_event_sender: &Sender<Event>,
) {
    ...
    create_jelly_terminal_thread(event_sender);
