...
impl App<'_> {
    ...
    fn render_user_input(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.user_input.is_empty() {
            let mut text = Text::from(Span::from("Type a command, for example: ").patch_style(Style::new().dark_gray()));
            text.push_span(Span::from(self.known_commands.list_by_cmd().join(", ")).patch_style(Style::new().dark_gray()));
        ...
        let (suggestion, cmds) = self.known_commands.longest_common_prefixed_by_cmd(&self.user_input);
        text.push_span(Span::from(suggestion.get(self.user_input.len()..).unwrap_or("")).patch_style(Style::new().dark_gray()));
        let command_options: String = match cmds.len() {
            0 => {
                String::new()
            },
            1 => {
                cmds[0].description.clone()
            },
            _ => {
                cmds.iter().map(|x| x.cmd.clone()).collect::<Vec<String>>().join(" ")
            }
        };
        text.push_span(
